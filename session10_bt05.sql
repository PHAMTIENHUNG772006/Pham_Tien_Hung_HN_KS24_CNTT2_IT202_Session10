drop database session10_bt04;
create database session10_bt04;
use session10_bt04;
...
-- trước '-> Limit: 10 row(s)  (actual time=0.503..0.507 rows=10 loops=1)\n    -> Sort: u.username DESC, p.post_id, limit input to 10 row(s) per chunk  (actual time=0.502..0.506 rows=10 loops=1)\n        -> Table scan on <temporary>  (cost=14.2..16.9 rows=25.3) (actual time=0.329..0.338 rows=83 loops=1)\n            -> Temporary table with deduplication  (cost=14.1..14.1 rows=25.3) (actual time=0.327..0.327 rows=83 loops=1)\n                -> Nested loop inner join  (cost=11.6 rows=25.3) (actual time=0.063..0.242 rows=83 loops=1)\n                    -> Filter: (u.hometown = \'Ha Noi\')  (cost=2.75 rows=2.5) (actual time=0.0175..0.0395 rows=8 loops=1)\n                        -> Table scan on u  (cost=2.75 rows=25) (actual time=0.0139..0.0305 rows=25 loops=1)\n                    -> Index lookup on p using posts_fk_users (user_id=u.user_id)  (cost=2.93 rows=10.1) (actual time=0.00824..0.0241 rows=10.4 loops=8)\n'

-- sau '-> Limit: 10 row(s)  (actual time=0.479..0.482 rows=10 loops=1)\n    -> Sort: u.username DESC, p.post_id, limit input to 10 row(s) per chunk  (actual time=0.477..0.48 rows=10 loops=1)\n        -> Table scan on <temporary>  (cost=37.9..41.4 rows=81) (actual time=0.298..0.309 rows=83 loops=1)\n            -> Temporary table with deduplication  (cost=37.9..37.9 rows=81) (actual time=0.297..0.297 rows=83 loops=1)\n                -> Nested loop inner join  (cost=29.8 rows=81) (actual time=0.0379..0.215 rows=83 loops=1)\n                    -> Index lookup on u using idx_hometown (hometown=\'Ha Noi\')  (cost=1.43 rows=8) (actual time=0.0246..0.0364 rows=8 loops=1)\n                    -> Index lookup on p using posts_fk_users (user_id=u.user_id)  (cost=2.66 rows=10.1) (actual time=0.00404..0.0213 rows=10.4 loops=8)\n'
-- thời gian đã được cải thiện 0.503 -> 0.479
