...
-- trước khi tạo index'-> Filter: (posts.created_at > TIMESTAMP\'2025-12-31 00:00:00\')  (cost=3.62 rows=6.2) (actual time=0.0327..0.0745 rows=8 loops=1)\n    -> Index lookup on posts using posts_fk_users (user_id=1)  (cost=3.62 rows=16) (actual time=0.0311..0.0711 rows=16 loops=1)\n'
explain analyze
select  post_id, content,created_at
from posts
where user_id = 1 and created_at > '2025-12-31';
-- sau khi có index'-> Index range scan on posts using idx_created_at_user_id over (user_id = 1 AND \'2025-12-31 00:00:00\' < created_at), with index condition: ((posts.user_id = 1) and (posts.created_at > TIMESTAMP\'2025-12-31 00:00:00\'))  (cost=3.86 rows=8) (actual time=0.0324..0.0564 rows=8 loops=1)\n'
-- có cost cao hơn nhưng số dòng lặp ít đi 1/2 và thời gian ít đi theo
...
-- trước '-> Rows fetched before execution  (cost=0..0 rows=1) (actual time=400e-6..400e-6 rows=1 loops=1)\n'
-- sau '-> Rows fetched before execution  (cost=0..0 rows=1) (actual time=200e-6..300e-6 rows=1 loops=1)\n'
-- => truy vấn nhanh gấp đôi 
