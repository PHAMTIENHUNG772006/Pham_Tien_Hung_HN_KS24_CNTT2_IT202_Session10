drop database session10_bt03;
create database session10_bt03;
use session10_bt03;
...
explain analyze
select username, hometown from users where hometown = 'Ha Noi';
-- '-> Filter: (users.hometown = \'Ha Noi\')  (cost=2.75 rows=2.5) (actual time=0.0358..0.0514 rows=8 loops=1)\n    -> Table scan on users  (cost=2.75 rows=25) (actual time=0.0304..0.0426 rows=25 loops=1)\n'
...
-- trước khi tạo index '-> Filter: (users.hometown = \'Ha Noi\')  (cost=2.75 rows=2.5) (actual time=0.0402..0.0607 rows=8 loops=1)\n    -> Table scan on users  (cost=2.75 rows=25) (actual time=0.0348..0.0517 rows=25 loops=1)\n'
explain analyze
select username, hometown from users where hometown = 'Ha Noi';
-- sau khi tạo '-> Index lookup on users using idx_hometown (hometown=\'Ha Noi\')  (cost=1.43 rows=8) (actual time=0.0486..0.0604 rows=8 loops=1)\n'
-- Cost giảm rõ rệt: 2.75 → 1.43 (giảm gần 50%)
-- Không cần quét toàn bộ bảng: Từ Table Scan → Index Lookup
-- Giảm số dòng đọc: Từ 25 dòng → 8 dòng (chỉ đọc đúng dữ liệu cần thiết)
